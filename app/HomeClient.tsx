...
// Quick spring for icons (only used for tap effects)
const tapSpring = {
  type: "spring" as const,
  stiffness: 500,
  damping: 25,
...
function ThemeDot() {
  ...
  return (
    ...
    >
      <span
        className="block w-3.5 h-3.5 rounded-full transition-transform duration-150 hover:scale-125"
...
function CloseButton({ onClick, variant = "default" }: { onClick: (e: React.MouseEvent) => void; variant?: "default" | "card" }) {
  ...
  if (variant === "card") {
    return (
      <motion.button
        onClick={onClick}
        className="fixed top-4 right-4 z-50 p-2 rounded-full cursor-pointer"
  ...
  return (
    <div className="blob-close-fixed">
      <motion.button
        onClick={onClick}
      ...
      <div className="flex items-center justify-center gap-10 md:gap-12">
        {navIcons.map(({ id, Icon, label }) => (
          <motion.button
            ...
            className="cursor-pointer p-4 -m-4 text-secondary transition-colors duration-150 hover:text-foreground"
            whileTap={{ scale: 0.85 }}
            animate={{
    ...
    <div className="social-grid">
      {data.social.map((social) => (
        <motion.a
          ...
          className="p-3 text-secondary transition-colors duration-150 hover:text-foreground"
          whileTap={{ scale: 0.9 }}
          transition={tapSpring}
