...
import { User, Share2, CreditCard, X, Github, Mail } from "lucide-react";
...
const navIcons = [
  ...
  { id: "socials" as const, Icon: Share2, label: "Socials" },
...
// Bouncy spring for blob appearance
const blobSpring = {
...
function CloseButton({ onClick, variant = "default" }: { onClick: () => void; variant?: "default" | "card" }) {
  const { resolvedTheme } = useTheme();

  // Better red pastel colors
  const bgColor = resolvedTheme === "dark" ? "rgba(255, 99, 99, 0.2)" : "rgba(220, 80, 80, 0.12)";
  const iconColor = resolvedTheme === "dark" ? "#FF8A8A" : "#D35555";

  return (
    <motion.button
      onClick={onClick}
      className={`p-2 rounded-full cursor-pointer z-10 ${variant === "card" ? "absolute top-4 right-4" : "absolute top-3 right-3"}`}
      ...
      aria-label="Close"
    >
      <X size={variant === "card" ? 20 : 16} style={{ color: iconColor }} />
    </motion.button>
  );
}
...
export default function HomeClient() {
  ...
  const handleIconClick = useCallback((section: Section) => {
  ...
  return (
    <div className="h-dvh flex items-center justify-center overflow-hidden">
      ...
      {/* Icons - Always visible, stay in place */}
      <div className="flex items-center justify-center gap-10 md:gap-12">
        {navIcons.map(({ id, Icon, label }) => (
          <motion.button
            key={id}
            onClick={() => handleIconClick(id)}
            className="cursor-pointer p-4 -m-4 text-secondary hover-hover:hover:text-foreground"
            whileHover={{ scale: 1.15 }}
            whileTap={{ scale: 0.85 }}
            animate={{
              scale: activeSection === id ? 1.1 : 1,
              opacity: activeSection && activeSection !== id ? 0.3 : 1,
            }}
            transition={iconSpring}
            aria-label={label}
          >
      ...
      {/* Content Overlay - Blob for about/socials, Card directly for card */}
      <AnimatePresence>
        {activeSection === "about" && (
          <motion.div
            key="about"
            className="blob blob-wide"
        ...
        {activeSection === "card" && (
          <motion.div
            ...
            transition={blobSpring}
          >
            <CloseButton onClick={handleClose} variant="card" />
            <Card />
          </motion.div>
        )}
      </AnimatePresence>
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-1">
        ...
        <div className="text-base md:text-lg font-medium tracking-tight text-secondary">
          {data.personal.title.join(" · ")}
        </div>
      </div>

      <div className="text-base md:text-lg font-medium tracking-tight text-secondary leading-relaxed">
        {data.personal.about}
      </div>
      ...
      <div className="space-y-1">
        ...
        <div className="space-y-0.5">
          {data.experience.map((exp, idx) => (
            <div
              key={idx}
              className="text-base md:text-lg font-medium tracking-tight text-secondary"
            >
    ...
    <div className="flex items-center justify-center gap-3">
      {data.social.map((social) => (
        <motion.a
          ...
          className="p-2.5 text-secondary hover-hover:hover:text-foreground"
